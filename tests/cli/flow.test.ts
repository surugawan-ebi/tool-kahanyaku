...
describe("kahanyaku CLI end-to-end flow", () => {
  ...
  it("runs init -> import -> list --pending -> approve -> search -> show -> export -> history", async () => {
    ...
    expect(fs.existsSync(path.join(project.dir, ".kahanyaku", "kahanyaku.sqlite"))).toBe(true);
    expect(fs.existsSync(path.join(project.dir, ".kahanyaku", "kahanyaku.config.yaml"))).toBe(true);
