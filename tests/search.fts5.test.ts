...
describe("createSearchEngine factory", () => {
  ...
  it('mode "fts5" (explicit) throws a clear KahanyakuError instead of silently falling back when unsupported', () => {
    const ctx = makeTestContext({ config: { search_engine: "fts5" } });
    ...
    expect(() => createSearchEngine(ctx)).toThrow(KahanyakuError);
    try {
      createSearchEngine(ctx);
      expect.unreachable();
    } catch (err) {
      expect(err).toBeInstanceOf(KahanyakuError);
      expect((err as KahanyakuError).code).toBe("invalid_input");
    }
  });
