...
export const GetContextPackInput = z.object({
  name: z.string().min(1).describe("citehanko.config.yamlのcontext_packsに定義されたpack名。"),
...
const DESCRIPTION = `[verified plane] citehanko.config.yamlのcontext_packsで定義された、verified noteの厳選済みセットを一括取得する。get_registry_overviewのcontext_packs[]で利用可能なpack名を確認できる。
