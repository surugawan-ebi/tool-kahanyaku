...
export type CiteHankoErrorCode = (typeof ERROR_CODES)[number];

export interface CiteHankoErrorOptions {
...
export class CiteHankoError extends Error {
  readonly code: CiteHankoErrorCode;
  readonly details?: Record<string, unknown>;
  readonly retryable: boolean;
  readonly suggested_action?: string;

  constructor(code: CiteHankoErrorCode, message: string, options: CiteHankoErrorOptions = {}) {
    super(message, options.cause !== undefined ? { cause: options.cause } : undefined);
    this.name = "CiteHankoError";
    this.code = code;
  ...
  toJSON(): { code: CiteHankoErrorCode; message: string; details?: Record<string, unknown>; retryable: boolean; suggested_action?: string } {
    return {
      code: this.code,
...
/**
 * Parses input against a zod schema, converting ZodError into the uniform CiteHankoError
...
export function parseOrThrow<Output>(schema: ZodType<Output, any, any>, input: unknown): Output {
  ...
  if (!result.success) {
    ...
    throw new CiteHankoError("invalid_input", message, { details: { issues: result.error.issues } });
  }
  return result.data;
}
