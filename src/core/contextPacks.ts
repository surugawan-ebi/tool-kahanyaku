  ...
  return {
    ...
    getPack(name: string, opts: GetContextPackOptions = {}): ContextPackResult {
      ...
      if (!pack) {
        ...
        throw new CiteHankoError("not_found", `context pack "${name}" was not found`, {
          details: { name },
          suggested_action:
            available.length > 0
              ? `available context packs: ${available.join(", ")}`
              : "no context packs are configured; add one under context_packs in citehanko.config.yaml",
        });
      }
