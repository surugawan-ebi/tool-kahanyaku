...
const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "citehanko-smoke-"));
let step = "startup";
let passed = 0;
...
fs.writeFileSync(
  path.join(dataDir, "citehanko.config.yaml"),
  `context_packs:
  smoke_pack:
...
async function main() {
  ...
  const transport = new StdioClientTransport({
    ...
    env: { ...process.env, CITEHANKO_ACTOR: "agent:smoke-test" },
    stderr: "pipe",
  });
  ...
  const client = new Client({ name: "citehanko-smoke-test", version: "0.0.0" }, { capabilities: {} });
  ...
  try {
    ...
    // context.ts's resolveDataDir), so other commands are pointed at it via CITEHANKO_HOME.
    const cliEnv = { ...process.env, CITEHANKO_HOME: dataDir };
